...
import { deleteOtherCostType } from './actions'
import type { OtherCostType } from '@/types/cost-type-lookup'

export default function OtherCostTypeList({ items }: { items: OtherCostType[] }) {
  ...
  function handleDelete(id: string, name: string) {
    if (!confirm(`Delete other cost type "${name}"? This cannot be undone.`)) return
    setDeletingId(id)
    startTransition(async () => {
      await deleteOtherCostType(id)
      setDeletingId(null)
    })
  }
              ...
              <tr>
                <td colSpan={2} className="px-4 py-16 text-center">
                  <p className="text-sm font-medium text-gray-400">
                    {items.length === 0 ? 'No other cost types defined yet.' : 'No types match your search.'}
                  </p>
                </td>
              </tr>
            ) : (
                ...
                <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <Link
                        href={`/admin/other-cost-types/${t.id}/edit`}
